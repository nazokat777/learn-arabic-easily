...
class _MasterDrillState extends State<MasterDrill> {
  ...
  List<int> _requiredModes(QiroatVocab v) =>
      _reqCache[_key(v)] ?? _computeRequiredModes(v);
  ...
  Widget _questionView() {
    ...
    final maskSum = _pool.fold<int>(
      0,
      (s, w) => s + progress.masterCount(_key(w)),
    );
  ...
  Widget _arWord(String ar, {bool play = false}) => Row(
    ...
    children: [
      ...
      if (play) ...[const SizedBox(width: 8), _playBtn(ar)],
    ],
  );

  Widget _uzWord(String uz) => Text(
    uz,
    textAlign: TextAlign.center,
    style: const TextStyle(
      fontSize: 24,
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              TextButton.icon(
                onPressed: (_built.isEmpty || _arrangeOk != null)
                    ? null
                    : () => setState(_built.clear),
              ...
              FilledButton(
                onPressed:
                    (_built.length != _target.length || _arrangeOk != null)
                    ? null
                    : _checkArrange,
                style: FilledButton.styleFrom(
        ...
        child: InkWell(
          ...
          child: Container(
            padding: EdgeInsets.symmetric(
              horizontal: isSentence ? 12 : 14,
              vertical: isSentence ? 8 : 10,
            ),
            child: Directionality(
              textDirection: TextDirection.rtl,
              child: Text(
    ...
    builder: (context, s, _) => Material(
      ...
      child: InkWell(
        ...
        child: const SizedBox(
          width: 84,
          height: 84,
          child: Icon(Icons.volume_up_rounded, color: Colors.white, size: 44),
        ),
      ),
    ),
  );
